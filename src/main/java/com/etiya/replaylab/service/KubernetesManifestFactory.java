...
public class KubernetesManifestFactory {
    ...
    public KubernetesManifestFactory(ReplayLabProperties properties) {
...
metadata:
  name: %s
  labels:
    app.kubernetes.io/managed-by: replaylab
---
apiVersion: v1
kind: ResourceQuota
metadata:
  name: replaylab-quota
  namespace: %s
spec:
  hard:
    requests.cpu: "2"
...
kind: Secret
metadata:
  name: replaylab-db-secret
  namespace: %s
type: Opaque
stringData:
  username: replaylab_user
  password: %s
  database: %s
---
apiVersion: v1
...
kind: Deployment
metadata:
  name: replaylab-postgres
  namespace: %s
spec:
  replicas: 1
  selector:
    matchLabels:
      app: replaylab-postgres
  template:
    metadata:
      labels:
        app: replaylab-postgres
...
%s      containers:
        - name: app
          ...
          env:
            - name: %s
              value: jdbc:postgresql://replaylab-postgres:5432/%s
            - name: %s
              valueFrom:
                secretKeyRef:
                  name: replaylab-db-secret
        ...
        - name: replay
          ...
          args:
            - >-
              ...
              http://app%s);
              echo REPLAYLAB_HTTP_STATUS=$STATUS;
              cat /tmp/body;
          volumeMounts:
            - name: request
              mountPath: /request
